...
def ADD_type():
    ...
    fenetre_Type.title("Products categories")
    ...
    label_ID = tk.Label(fenetre_Type, text="Reference categorie:", bg="#ffff00")

    entry_ID = tk.Entry(fenetre_Type)

    label_nom = tk.Label(fenetre_Type, text="Nom  de  categorie:", bg="#ffff00")

    entry_nom = tk.Entry(fenetre_Type)

    def valider_ajout():
        Type_ID = entry_ID.get()
        Nom = entry_nom.get()

        # Vérifier si les champs ID et nom sont remplis
        if Type_ID and Nom:
            # Insérer les données dans la table "Fournisseur"
            sql = "INSERT INTO type (Type_ID, Nom) VALUES (%s, %s)"

            values = (Type_ID, Nom)
            cursor.execute(sql, values)
            conn.commit()

            messagebox.showinfo("Succès", "Le type a été ajouté avec succès.")
    ...
    label_type = tk.Label(fenetre_Type, text="Type deja disponible:", bg="paleturquoise")
    ...
    try:
        ...
        messagebox.showerror("Erreur", f"Erreur lors de la récupération des fournisseurs : {err}")
        return
    ...
    label_nom.place(relx=0.23, rely=0.4, anchor=tk.CENTER)
    entry_nom.place(relx=0.35, rely=0.4, anchor=tk.W)
